...
public class Prodi extends javax.swing.JFrame {
    ...
    private void TOMBOL_HAPUSActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_TOMBOL_HAPUSActionPerformed
        // TODO add your handling code here:
           // Ambil kode prodi dari form
    String kodeProdi = KODE_PRODI.getText().trim();

    // Validasi input
    if (kodeProdi.isEmpty()) {
        JOptionPane.showMessageDialog(null, "Silakan masukkan Kode Prodi yang ingin dihapus.");
        return;
    }
 ...
 if (rs.next()) {
            // Jika ada, hapus data
            String deleteQuery = "DELETE FROM prodi WHERE kode_prodi = '" + kodeProdi + "'";
            int hasil = st.executeUpdate(deleteQuery);
            if (hasil > 0) {
        ...
        while (rsRefresh.next()) {
            String kode = rsRefresh.getString("kode_prodi");
            String nama = rsRefresh.getString("nama_prodi");
            String pass = rsRefresh.getString("password");
            int jumlah = rsRefresh.getInt("jumlah_dosen");
    ...
    try {
        ...
        Class.forName("com.mysql.cj.jdbc.Driver");
        con = DriverManager.getConnection("jdbc:mysql://localhost:3306/unpam", "root", "");

        // Path ke file .jrxml
        String jrxmlPath =  System.getProperty("user.dir") + "/src/com/program/DataReport.jrxml";

        // Compile .jrxml ke JasperReport object
        JasperReport report = JasperCompileManager.compileReport(jrxmlPath);

        // Isi laporan (gunakan HashMap untuk parameter jika ada, null jika tidak)
        JasperPrint print = JasperFillManager.fillReport(report, new HashMap<>(), con);
        // Tampilkan laporan
        JasperViewer viewer = new JasperViewer(print, false);
        viewer.setTitle("Laporan Data");
